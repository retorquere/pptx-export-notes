...
def run():
  ...
  parser.add_argument('-p', metavar='<path/to/pptx/file>', help='path to the Powerpoint 2007+ file', action='store', type=argparse.FileType('r'), dest='pptxfile')
  ...
  writepath = os.path.dirname(args.pptxfile.name) + '/' + os.path.basename(args.pptxfile.name).rsplit('.', 1)[0] + '_presenter_notes.html'
  print(writepath)
  ...
  with tag('html'):
    with tag('head'):
      with tag('style'):
        text('table { border-collapse: collapse; } table, th, td { border: 1px solid black; text-align:left; vertical-align:top; }')
    with tag('body'):
      with tag('table'):
        ...
        for x in [key for key in sorted(notesDict.keys(), key=int)]:
          ...
          notes_string = notes_string.encode('utf-8', 'ignore')
          notes_string = re.sub(r'' + str(x) + r'$', '', notes_string)
          if notes_string.strip() == '': continue
